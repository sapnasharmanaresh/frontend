...
class LiveMatchesListTest extends FreeSpec with ShouldMatchers with MatchTestData with Football with OptionValues {
  ...
  "the competition live matches list" - {
    val matches = new CompetitionLiveMatchesList(competitions, "1")

    "should be showing the correct live matches from the test data" in {
      matches.relevantMatches.map { case (fmatch, _) =>
        fmatch.id
      }.sortBy(_.toInt) should equal(List("5", "6"))
    }
